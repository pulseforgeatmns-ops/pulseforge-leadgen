...
function resolveClientId(req, body = {}) {
  ...
  if (!Number.isInteger(clientId) || clientId < 1) throw new ContentLearningError('client_id_required', 'A valid client is required.');
  const access = assertAuthorizedClientSwitch(req.user, clientId);
  if (!access.ok) throw new ContentLearningError(access.error, 'Client scope is not authorized.', access.status);
  if (body.tenantId != null && String(body.tenantId) !== String(clientId)) throw new ContentLearningError('tenant_client_mismatch', 'Tenant and client must match.');
  return clientId;
}

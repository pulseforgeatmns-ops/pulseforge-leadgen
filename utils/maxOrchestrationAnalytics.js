...
async function getShadowQualityAnalytics({ db = pool, clientId, days = 30 } = {}) {
  const [scores, states, components, reasons, channels, timing, oscillation, blocked, outcomes] = await Promise.all([
    ...
    db.query(`SELECT metric_name, AVG(metric_value)::numeric(12,2) AS average_ms FROM max_orchestration_metrics WHERE client_id=$1 AND metric_name IN ('live_signal_to_decision_latency','live_processing_latency','historical_backfill_processing_latency','manual_recalculation_processing_latency','decay_processing_latency','decision_processing_duration') AND recorded_at>=NOW()-($2::int*INTERVAL '1 day') GROUP BY metric_name`, [clientId, days]),

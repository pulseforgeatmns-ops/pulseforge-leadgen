...
const RESULT_COPY = {
  call_recovery_gap: {
    ...
    meaning:
      'A missed caller is a live buying opportunity — someone already raised their hand. When no one texts or calls back quickly, they usually hire whoever responds first.',
    first_move:
      'In the next 15 minutes: write a short missed-call text-back (“Sorry we missed you — when’s a good time to talk about cleaning?”) and decide who sends it every time.',
  },
  quote_follow_up_gap: {
    ...
    meaning:
      'Sending a quote is not the same as working the quote. Most cleaning buyers need a clear, polite nudge before they decide — silence reads like you’re too busy or not interested.',
    first_move:
      'In the next 15 minutes: pull every open quote from the last 14 days and send one simple follow-up (“Want me to hold this on the schedule, or revise anything?”).',
  },
  review_growth_gap: {
    ...
    meaning:
      'Happy jobs need to become visible proof for the next buyer. When review asks depend on memory, great work stays private — and the next prospect has less reason to trust you.',
    first_move:
      'In the next 15 minutes: message your last five satisfied customers with a direct review request and your Google (or preferred) review link.',
  },
};

/**
 * Build the job-value illustration line from the visitor’s selected range.
 * Uses range language only — never a fake precise amount or conversion rate.
 */
function formatJobValueIllustration(typicalJobValue) {
  const label = JOB_VALUE_LABELS[typicalJobValue];
  if (!label) {
    return 'If this process helps recover even one additional job, that could mean meaningful booked work for your business. This is an illustration, not a promise.';
  }
  return `If this process helps recover even one additional job, that could mean approximately ${label} in booked work. This is an illustration, not a promise.`;
}
...
function resolveResult(answers) {
  ...
  return {
    ...
    payoff: {
      ...
      first_move: copy.first_move,
      job_value_key: answers.typical_job_value || null,
      job_value_illustration: formatJobValueIllustration(answers.typical_job_value),
      recovery_plan: [...RECOVERY_PLAN_STEPS],
    },
  };
}

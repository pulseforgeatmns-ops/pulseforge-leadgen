...
async function reconstructCanonicalSemanticProjection(pool, input = {}) {
  ...
  try {
    ...
    const projectedEntities = sorted(entities).map(entity => {
      const assertions = [...(labelsByEntity.get(entity.id) || [])].sort((a, b) => `${a.created_at}:${a.id}`.localeCompare(`${b.created_at}:${b.id}`));
      const canonical = assertions.filter(row => row.assertion_kind === 'CANONICAL').at(-1);

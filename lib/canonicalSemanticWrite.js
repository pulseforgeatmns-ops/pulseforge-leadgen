...
async function commitCanonicalSemanticBatch(pool, input) {
  ...
  try {
    ...
    const merges = await appendMerges(client, batch, batchId, entities.refs, evidenceInputs, prior?.manifest);
    const conflicts = await resolveConflicts(client, batch, facts, relations, prior?.manifest);
    const base = prior?.registry_artifact_id === batch.registry_artifact_id ? prior.manifest : {};

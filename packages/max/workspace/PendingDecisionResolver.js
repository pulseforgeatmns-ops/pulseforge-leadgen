...
function resolvePendingOperatorDecision(question, mission) {
  ...
  if (isHoldOrDefer(q)) {
    return { resolved: false, outcome: RESOLUTION_OUTCOMES.AMBIGUOUS };
  }

  if (isUnrelatedTopic(q)) {
    return { resolved: false, outcome: RESOLUTION_OUTCOMES.UNRELATED };
  }

  if (isQuestionAboutDecision(q)) {
    return { resolved: false, outcome: RESOLUTION_OUTCOMES.QUESTION };
  }

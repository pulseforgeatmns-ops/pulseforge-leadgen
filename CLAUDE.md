...
| `routes/tenantWorkspace.js` | SPEC-114 create/activate tenant, workspace snapshot, Max tenant context |
| `services/tenantWorkspace.js` | Tenant provision + isolated namespace status |
| `public/admin-clients.html` | Operator Create Client / Activate workspace |
| `routes/substralAssessment.js` | `POST /api/public/website-assessment` — Studio Substral site intake. Captures a request only; never returns a finding about the domain. |
| `lib/substralAssessmentIntake.js` | Domain admission + validation + `agent_actions` capture for the Studio Substral assessment request. Reuses the WOI engine's `discoveryAdmission` rules. |
| `sites/studio-substral/` | Studio Substral website (static). `DOCTRINE.md` is the creative source of truth; `DOCTRINE-IMPLEMENTATION.md` is the engineering record. |
...
| 10 | Anchor Cleaning | — | Manchester NH | Slug `cleaning-co`. **Separate LLC — commercial-cleaning BUYER, not a Pulseforge lead-gen client.** `scoring_profile='cleaning_buyer'` (its own ICP rubric — see Known Architectural Notes). Scout targets law firms + accounting practices in the Greater Manchester NH ring. `enabled_agents=['scout']` only (no Emmett/social until entity name, domain, and sender are set). `service_area` = Manchester ~20mi ring (radius is a default pending Jacob's confirmation). Entity name set to Anchor Cleaning (Jun 2026); service-area radius still TBD. Added Jun 2026. |
...
- **Tenant create/activate**: `POST /api/clients`, `GET /api/clients/:id/workspace`, `GET /api/v1/tenant/context`, `POST /api/v1/tenant/activate`, `GET /admin/clients` — in `routes/tenantWorkspace.js`
...
- **Public site intake (unauthenticated)**: `POST /api/public/walkthrough` (Anchor Cleaning) — in `routes/walkthrough.js`; `POST /api/public/website-assessment` (Studio Substral) — in `routes/substralAssessment.js`. Both rate-limited and honeypot-guarded, both write a `pending` `agent_actions` row.
...
- **Per-client ICP scoring profiles (added Jun 2026)** — `clients.scoring_profile` selects which ICP rubric Scout's `scoreLead()` applies. NULL = the default Pulseforge lead-gen rubric (vertical/location/contact/web/size, plus MSHI `client_id=2` boosts and probate weighting). `'cleaning_buyer'` = a fully separate rubric (`scoreCleaningLead` in `leadgen.js`) for the **cleaning company (`client_id=10`)**, which scores commercial-cleaning BUYERS, not lead-gen targets. The cleaning rubric drops ALL Pulseforge signals (probate, MSHI, tech/web-presence) and scores: vertical match 0–35 (single-tenant professional-services office — law firm / CPA practice), geography 0–25 (Manchester pilot cluster), reachable decision-maker 0–25 (owner/office-manager name + email/phone), single-tenant signal 0–10 (address-derived heuristic — flagged as a limitation when undetectable), size proxy 0–5 (prefer small offices — flagged when thin), minus a **disqualifier penalty**: a detectable multi-office/national firm (−25) and/or a high-floor/large-suite multi-tenant tower address (−12) actively subtract so the wrong beachhead falls BELOW threshold instead of merely zero-awarding (e.g. a Beacon-National-type lead: 72 → 35). The single-tenant address heuristic is intentionally left as-is for the ambiguous middle (those are filtered at walkthrough booking — do not over-engineer detection). `CONFIG.scoringProfile` is loaded from `CLIENT_CONFIG.scoring_profile` in `run()`. Setter visibility uses the global `icp_score >= 70` threshold for every client, including client 10; the authority lives in `utils/setterVisibility.js`. To tune the rubric, edit `scoreCleaningLead`; eyeball it offline with `node scripts/testCleaningScout.js`, and report a live pass with `node scripts/cleaningPilotReport.js` (no DB/API needed for the former; DB for the latter).
- **Cleaning client Scout config (`client_id=10`)** — Google Places is **PRIMARY** for this client (SerpAPI is skipped — it underperforms on professional offices and runs dry); see the `placesPrimary` branch in `leadgen.js`'s search loop. Law firms and accounting practices run as **separate passes, one vertical per run** (`CLIENT_SCOUT_PLANS[10]`, verticals `law_firm` and `accounting`). Prospeo title filter is widened to owner OR office manager (plus principal/partner/founder/president) via `getProspeoTitleIncludes()`. Out-of-area leads are culled by the existing `service_area` gate (`clients.service_area` = Manchester pilot cluster: Manchester, Bedford, Goffstown, Hooksett, Londonderry, Auburn — wider ring held for post-pilot; keep in sync with `CLEANING_AREA_CITIES`). **Run the test pass first** — `node leadgen.js --client_id 10 --industry "law firm" --location "Manchester NH" --max 15 | tee /tmp/cleaning-pass.log` then `node scripts/cleaningPilotReport.js --vertical law_firm` — and let Jacob validate quality/scoring/enrichment-hit-rate before any full multi-vertical run. **Partitioning note:** setter users must have `client_id` assigned (e.g. William = 1) so client-10's `setter_visible` leads never surface in another client's setter queue — the setter queue only scopes by client when `session.user.client_id` is set.
...
- **Multi-client architecture (added May 2026)** — all primary data tables carry `client_id` FK to `clients`. Agents accept `client_id` via `POST /api/run/:agent?client_id=1` or `/cron/:agent?client_id=2` and scope DB reads/writes accordingly. Dashboard has a client selector in the header and stores the active client in `req.session.active_client_id`. Existing data backfilled to `client_id=1` (Pulseforge). MSHI is `client_id=2`.
- **Client tenant creation (SPEC-114, added Aug 2026)** — operators create a new tenant from `/admin/clients` or `POST /api/clients`. PulseForge provisions isolated knowledge/mission/prospect/outcome/AIM namespaces. The workspace starts empty (no Client Intelligence, no published AIM). Activate via the shell client selector or `POST /api/v1/tenant/activate`. Max fail-closes with `No active client selected.` when no tenant is active. Do **not** `INSERT INTO clients` by hand to start Pilot 0 (Fedir). Do **not** attach the hand-authored Fedir AIM seed to a newly created Fedir tenant — compile and publish through SPEC-113. Outreach agents stay off (`enabled_agents=['max']`) until intentionally enabled. Cron/email-sequence wiring for a live outreach client is a later step, not part of provision.
...
- **Studio Substral site (added Sep 2026)** — `sites/studio-substral/` is a separate customer-facing brand, not a PulseForge advertisement, and the page copy must never mention PulseForge (a test enforces this). Its Act IV evidence taxonomy and four diagnosis classes are read from `packages/capabilities/websiteOpportunityIntelligence/types.js`, and the engine's `PROHIBITED_CLAIM_PATTERNS` are asserted against the page copy, so the marketing surface cannot claim more than the capability can produce. **No score is ever produced.** `test/studioSubstralDoctrine.test.js` checks the source; `sites/studio-substral/build/verify-layout.mjs` drives a browser and is the check that matters after any stylesheet, markup or object change. `assets/js/dimensional.js` and the brand rasters are **generated** — edit `src/dimensional.js` / `assets/brand/favicon.svg` and rebuild from `build/`. The domain, mailbox and intake origin are placeholders; see the launch-blocker table in `DOCTRINE-IMPLEMENTATION.md`.
